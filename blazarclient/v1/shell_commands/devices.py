...
class CreateDevice(command.CreateCommand):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--device-driver", default="zun", help="The driver of the device"
        )
        parser.add_argument(
            "--extra",
            metavar="<key>=<value>",
...
class UpdateDevice(command.UpdateCommand):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument("--device-driver", help="The driver of the device")
        parser.add_argument(
            "--extra",
            metavar="<key>=<value>",
            action="append",
...
class UnsetAttributeDevice(UpdateDevice):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            default=[],
            help=(
                "Extra capabilities key/value pairs"
                " to update for the device",
            )
        )
        return parser
    ...
    def args2body(self, parsed_args):
        if parsed_args.extra_capabilities:
            return {
                "values": {cap: None for cap in parsed_args.extra_capabilities}
            }
        else:
            return {}
...
class ReallocateDevice(command.ReallocateCommand):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--lease-id", help="Lease ID to reallocate device from."
        )
        parser.add_argument(
            "--reservation-id", help="Reservation ID to reallocate device from"
        )
        return parser
...
class ListDeviceCapabilities(command.ListCommand):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--sort-by",
            metavar="<extra_capability_column>",
            help="column name used to sort result",
            default="property",
        )
        return parser
