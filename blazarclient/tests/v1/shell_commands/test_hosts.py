...
class UpdateHostTest(tests.TestCase):
    ...
    def test_update_host(self):
        ...
        args = argparse.Namespace(
            id="101", extra_capabilities=["key1=value1", "key2=value2"]
        )
    ...
    def test_update_host_with_name(self):
        ...
        update_host, host_manager = self.create_update_command(list_value)
        args = argparse.Namespace(
            id="host-1", extra_capabilities=["key1=value1", "key2=value2"]
        )
        expected = {"values": {"key1": "value1", "key2": "value2"}}
        update_host.run(args)
        host_manager.update.assert_called_once_with("101", **expected)
    ...
    def test_update_host_with_name_startwith_number(self):
        ...
        update_host, host_manager = self.create_update_command(list_value)
        args = argparse.Namespace(
            id="1-host", extra_capabilities=["key1=value1", "key2=value2"]
        )
        expected = {"values": {"key1": "value1", "key2": "value2"}}
        update_host.run(args)
        host_manager.update.assert_called_once_with("101", **expected)
...
class UnsetAttributesHostTest(tests.TestCase):
    def create_unset_command(self, list_value):
        ...
        blazar_shell.client = mock_client
        return (
            hosts.UnsetAttributesHost(blazar_shell, mock.Mock()),
            mock_host_manager,
        )
    ...
    def test_unset_host(self):
        ...
        args = argparse.Namespace(id="101", extra_capabilities=extra_caps)
...
class ShowHostTest(tests.TestCase):
    ...
    def test_show_host(self):
        ...
        show_host, host_manager = self.create_show_command(
